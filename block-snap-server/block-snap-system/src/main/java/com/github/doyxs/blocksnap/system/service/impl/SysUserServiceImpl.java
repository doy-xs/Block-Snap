...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public Result<String> bindAccount(Long userId, BindAccountDTO dto) {
        ...
        if (this.count(uniqueQuery) > 0) throw new ApiException("该" + (isEmail ? "邮箱" : "手机号") + "已被其他账号绑定");
    ...
    public Result<String> forgotPassword(ForgotPasswordDto dto) {
        ...
        SysUser user = this.getOne(new LambdaQueryWrapper<SysUser>()
                .and(w -> w.eq(SysUser::getPhone, account).or().eq(SysUser::getEmail, account)));
        ...
        if (!StringUtils.equals(dto.getResetPassword(), dto.getConfirmResetPassword())) throw new ApiException("密码不一致");
